"""Render a non-executable single-cell WGS planning brief."""
...
def render_manual_markdown(p: Params) -> str:
    ...
    return f"""# single-cell WGS workflow planning brief

> RESEARCH USE ONLY. Simulation only. This document is not a bench protocol.

Requested synthetic samples: **{p.n_samples}**
...
The sample manifest, paired reads, pipeline checkout, and execution profile are
provided at runtime.
"""
...
def write_manual(p: Params, out_dir: Path | str) -> dict[str, str | None]:
    ...
    path = out / "wgs_planning_brief.md"
