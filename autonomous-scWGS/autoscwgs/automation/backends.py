...
`mode="sim"`      -> hardware-free simulation (chatterbox backends + a dsDNA signal
                     model for the plate reader). Runs anywhere.
`mode="hardware"` -> resolves the real backends named in instruments.yaml
                     (STARBackend / ODTC backend / SynergyH1Backend). Refuses to run
                     if the registry still has TODO placeholders.
"""
...
from .qubit import QubitSimBackend
...
def _sim_bundle(p: Params) -> BackendBundle:
    ...
    return BackendBundle(
        ...
        pr_backend=QubitSimBackend(seed=p.n_samples),  # deterministic per run size
...
def _resolve_hardware(p: Params) -> BackendBundle:  # pragma: no cover - needs hardware
    ...
    tc_name = _check(inst["hamilton_odtc"]["plr"].get("backend_hardware"), "hamilton_odtc.backend_hardware")
