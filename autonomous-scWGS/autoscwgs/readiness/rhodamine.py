...
Why: before spending irreplaceable single cells, prove the Hamilton STAR dispenses
...
constant final volume, read fluorescence on the H1, and compute the per-well CV.
Precision (CV) is the readiness gate the user asked for; mean/SD are reported too.

This is Stage 0 and is REQUIRED (fail-closed) before the WGA/library-prep run.
...
(Design shared with flashseq-skill's readiness module -- same series.)
"""
    ...
    def summary(self) -> str:
        ...
        for r in self.ranges:
            ...
            lines.append(
                ...
                f"{r.cv_pct:>6.2f} {r.cv_threshold_pct:>6.1f}  [{mark}] ({r.represents[:40]})"
            )
        lines.append("-" * 72)
